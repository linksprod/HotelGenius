...
import { Plus, RefreshCw, Utensils, LayoutGrid, List, Trash2 } from 'lucide-react';
...
const RestaurantCard = ({ restaurant, onEdit, onDelete, onViewReservations, onViewMenus, onAddEvent, onTogglePublish, allowsBooking }) => {
  // Real occupancy and busyness could be calculated here in the future
  const occupancy = 0;
  const isBusy = false;
  ...
  const images = restaurant.images && restaurant.images.length > 0 
    ? restaurant.images 
    : [restaurant.image_url || 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=2070&auto=format&fit=crop'];
      ...
      <div className="aspect-video relative overflow-hidden group/img">
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/60 text-white flex items-center justify-center hover:bg-black/80 transition-all opacity-0 group-hover/img:opacity-100 font-bold z-30 text-xs"
            >
              ‹
            </button>
            <button
              onClick={nextImage}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/60 text-white flex items-center justify-center hover:bg-black/80 transition-all opacity-0 group-hover/img:opacity-100 font-bold z-30 text-xs"
            >
              ›
            </button>
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 bg-black/40 px-1.5 py-0.5 rounded-full z-30">
              {images.map((_, i) => (
                <div 
                  key={i} 
        ...
        <div className="absolute top-3 left-3 z-20">
          <Button
            ...
            className="h-7 w-7 rounded-lg bg-red-600/90 hover:bg-red-700 text-white border-none shadow-lg flex items-center justify-center shrink-0"
            title="Delete Venue"
          >
        ...
        <div className="absolute top-3 right-3 flex flex-col items-end gap-2 z-20">
          <div className="flex gap-2">
            <Badge 
              className={`cursor-pointer border-none text-[10px] font-bold ${restaurant.is_published ? 'bg-accent/80 hover:bg-accent text-accent-foreground' : 'bg-amber-500 hover:bg-amber-600 text-white shadow-lg shadow-amber-500/20'}`}
              onClick={(e) => {
                e.stopPropagation();
                onTogglePublish(restaurant);
              }}
            >
              {restaurant.is_published ? 'PUBLISHED' : 'DRAFT'}
            </Badge>
            <Badge className="bg-emerald-500 text-white border-none text-[10px] font-bold pointer-events-none">OPEN</Badge>
          </div>
          {isBusy && (
...
const RestaurantManager = () => {
  ...
  const allowsBooking = hotel?.plan ? hotel.plan !== 'essential' : true;
  ...
  const handleTogglePublish = async (restaurant) => {
    try {
      ...
      toast.success(restaurant.is_published ? 'Moved to Draft' : 'Published Successfully');
  ...
  const handlePublishAll = async () => {
    try {
      setIsRefreshing(true);
      const drafts = restaurants?.filter(r => !r.is_published) || [];
      if (drafts.length === 0) {
        toast.info("No drafts found to publish.");
        setIsRefreshing(false);
        return;
      }
      ...
      if (error) throw error;
      toast.success(`Published ${drafts.length} venues successfully`);
      queryClient.invalidateQueries({ queryKey: ['restaurants'] });
    } catch (error) {
      ...
      >
        ...
        >
          <AdminPageHeader
            ...
            description="Manage restaurants, bookings, and menus"
            actions={
              <>
                <Button 
                  onClick={handlePublishAll}
                  disabled={isRefreshing || !restaurants?.some(r => !r.is_published)}
