...
type SecurityRequestsTabProps = {
  categoryIds: string[];
};

const SecurityRequestsTab = ({ categoryIds }: SecurityRequestsTabProps) => {
  const [searchTerm, setSearchTerm] = useState('');
  const { toast } = useToast();
  const { requests, isLoading, handleRefresh } = useRequestsData();
  const { highlightedId, highlightRef } = useHighlightedRequest();
  
  const securityRequests = requests.filter(
    request => 
      (categoryIds.includes(request.category_id || '') || request.type === 'securite') &&
      (
...
export default SecurityRequestsTab;
