...
async function verifyFix() {
    ...
    const TEST_USER_ID = "3e4af4c8-67eb-4783-866c-fb0fcc8dd265";

    console.log(`Verifying fix for Fiesta (${FIESTA_ID}) and user (${TEST_USER_ID})...`);

    // 0. Find a room_id for Fiesta or create a dummy if needed
    // Actually let's just pick ANY room for now if rooms doesn't have hotel_id
    const { data: room } = await supabase.from('rooms').select('id').limit(1).single();
    if (!room) {
        console.error("No rooms found in database. Cannot create request.");
        return;
    }
    console.log("Using room_id:", room.id);

    // 1. Ensure guest is associated with Fiesta
    console.log("Ensuring guest-hotel association...");
    const { data: existingGuest } = await supabase.from('guests').select('id, hotel_id').eq('user_id', TEST_USER_ID).maybeSingle();

    if (existingGuest) {
        console.log("Updating existing guest:", existingGuest.id);
        await supabase.from('guests').update({ hotel_id: FIESTA_ID }).eq('id', existingGuest.id);
    } else {
    ...
    if (vError) {
        ...
        if (found) {
            console.log("SUCCESS: The resilient visibility fix is working!");
        }
    }
