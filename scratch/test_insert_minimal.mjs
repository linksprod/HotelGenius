...
async function run() {
  ...
  const payload = {
    ...
    title: "Test guest request minimal",
    body: "Test guest body minimal",
    priority: "normal",
    source_module: "Service",
    source_event: "created",
    reference_id: "68003931-fca8-4dbd-8f1b-e0b85d1e0cb0",
    reference_type: "ServiceRequest",
    idempotency_key: "test-guest-insert-minimal-" + Date.now(),
    status: "pending"
  };

  console.log("Inserting notification without .select()...");
  const { error } = await client
    .from('notifications')
    .insert(payload);

  if (error) {
    console.error("INSERT WITHOUT SELECT FAILED WITH ERROR:");
    console.error(error);
  } else {
