...
async function check() {
  const { data: hotels, error: hErr } = await supabase.from('hotels').select('id, name, slug');
  console.log("Hotels:", hotels);

  const { data: abouts, error: aErr } = await supabase.from('hotel_about').select('id, hotel_id, loyalty_enabled, loyalty_title, loyalty_description, loyalty_tiers, loyalty_benefits');
  console.log("Hotel About rows:", abouts);
}
