...
async function check() {
  ...
  const emna = users.users.find(u => u.email === 'emna.jmal@gmail.com');
  console.log("Emna Auth User:", emna);

  const { data: roles } = await supabase.from('user_roles').select('*').eq('user_id', emna?.id);
  console.log("Emna Roles in DB:", roles);
}
